...
from .image_io import get_image_size, load_image
from .bounding_box.visualization import normalize_color, box_as_mask
...
def create_poly_mask(image, points, color = 1., dtype = np.float32):
    ...
    # `cv2.fillPoly` expects a numeric scalar / tuple, not a `np.ndarray`
    color   = normalize_color(color, image = image).tolist()
...
def apply_mask(image,
    ...
    """
        ...
        Arguments :
            ...
            - on_background : whether to apply transformation on the mask or on its background
...
def mask_boxes(image, boxes, *, method = 'keep', ** kwargs):
    """
        Masks `image` based on the region covered by `boxes`

        Builds a (filled) box mask via `box_as_mask` then applies it with `apply_mask`
        (e.g. `keep` preserves the boxes' content, `remove` blanks it, `blur` blurs it, ...)

        Arguments :
            - image  : the image to mask (raw array or filename)
            - boxes  : the boxes (any format supported by `convert_box_format` ; a `dict` with
                       a `format` entry provides the `source`)
            - method : the transformation forwarded to `apply_mask`
            - kwargs : forwarded to both `box_as_mask` (drawing) and `apply_mask`
        Return :
            - the masked image with same shape as `image`
    """
    ...
    return apply_mask(image, mask, method = method, ** kwargs)
